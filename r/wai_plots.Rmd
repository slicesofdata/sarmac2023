---
title: "weighted-average illusion plots"
#author: "gcook"
date: Sys.Date()
...
r_dir    <- here::here("r")    # r subdir
data_dir <- here::here("data")

source(here::here("r", "src", "functions.txt"))
source(here::here("r", "src", "code.txt"))
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
DAT <- read.csv(paste0(dat_dir, "/jurorRandValue-1-31-numeric.csv"), 
                        header = T)
DAT <- DAT[ -c(1,2),]
NC = DAT[ , grep("NC_", names(DAT))]
DJ = DAT[ , grep("DJ",  names(DAT))]

DAT <- read.csv(paste0(dat_dir, "/jurorRandValue-1-31.csv"), header = T)
qual_vars = c("Crime", "Age", "Vote", "Jury", "Educ", "Job", "Income", 
              "Political", "Race", "Sex",
...
DAT <- DAT %>% 
  ...
  dplyr::mutate(
    ...
    RiskCheck_Not_Rem = ifelse(RiskCheck == "Do not remember", "Not Remember", ""), # clean up
    RiskCheck = ifelse(RiskCheck == "Do not remember", NA, RiskCheck),
    ...
    RiskCheckAcc  = ifelse(as.numeric(RiskValue) == as.numeric(RiskCheck), 1, 0),
