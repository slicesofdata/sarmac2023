...
```{r}
reduceBy = .10 # so that the plot is more manageable 
BLAIS_2y_obs <- data.frame(
  score = c(   rep(-2, 82*reduceBy), 
               rep(-1, 17*reduceBy), 
               rep( 0, 179*reduceBy), 
               rep( 1, 179*reduceBy), 
               rep( 2, 251*reduceBy), 
  ...
  rate = c(
    ...
    rnorm(179*reduceBy, 3.5,  2), 
    rnorm(179*reduceBy, 5.7,  2), 
    rnorm(251*reduceBy, 9.0,  3), 
    rnorm(156*reduceBy, 14.0, 3), 
    rnorm(174*reduceBy, 21.2, 4), 
    rnorm(153*reduceBy, 30.6, 4), 
    rnorm(150*reduceBy, 42.0, 8),
    rnorm(111*reduceBy, 54.4, 8), 
    rep(NA, 6*reduceBy)
    ),
  # observed
  rate_obs = c(
    rnorm(82*reduceBy,  1.2,  1),
    rnorm(17*reduceBy,  0,    1), 
    rnorm(179*reduceBy, 3.4,  2), 
    rnorm(179*reduceBy, 5.6,  2), 
    rnorm(251*reduceBy, 10.8, 3), 
    rnorm(156*reduceBy, 9.6,  3), 
    rnorm(174*reduceBy, 19.0, 4), 
    rnorm(153*reduceBy, 25.5, 4), 
    rnorm(150*reduceBy, 36.0, 8),
    rnorm(111*reduceBy, 53.2, 8),
    rnorm(6*reduceBy, 50, 8)
    ),
  rate_bin = c(
...
write.csv(BLAIS_2y_obs, 
          here::here("data", paste0(
            "BLAIS_2y_obs_seed - ", seed, ".csv"))
)
          
BLAIS_2y_obs <- read.csv(
  here::here("data", paste0(
            "BLAIS_2y_obs_seed - ", seed, ".csv"))
)
...
Blais.p.unweighted <- ggplot(data = BLAIS_2y_obs, 
...
Blais.p.weighted <- ggplot(data = BLAIS_2y_obs, 
                  mapping = aes(x = score, y = rate
                                , #alpha = rate
                                )) +
  # for wai1, size 2, width was .7; .3 for wai2, alpha = .5
...
#str(BRENT)
#BRENT
write.csv(BRENT, paste0("data/BRENT_2y_score2_seed - ", seed, ".csv"))
BRENT <- read.csv(paste0("data/BRENT_2y_score2_seed - ", seed, ".csv"))
